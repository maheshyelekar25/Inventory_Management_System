...
import { createProduct, deleteProduct, getCategories, getProducts, updateProduct } from '@/api/inventory'
...
export function ProductsPage() {
  ...
  const loadProducts = async () => {
    ...
    setError(null)
    try {
      const response = await getProducts({ page: 1, page_size: 100, search: debouncedSearch || undefined })
  ...
  useEffect(() => { void loadProducts() }, [debouncedSearch])
  ...
  return <section className="space-y-6"><div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><h1 className="text-2xl font-bold tracking-tight">Products</h1><p className="mt-1 text-slate-600">Create, update, and track your inventory.</p></div><button onClick={() => setModalProduct(null)} className="rounded-md bg-brand-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-brand-700">+ Add product</button></div><section className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm"><div className="flex flex-col gap-3 border-b border-slate-200 p-4 sm:flex-row sm:items-center sm:justify-between"><label className="w-full sm:max-w-sm"><span className="sr-only">Search products</span><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search products or SKU" className="block w-full rounded-md border border-slate-300 px-3 py-2 text-sm outline-none focus:border-brand-600 focus:ring-2 focus:ring-brand-50" /></label><p className="text-sm text-slate-500">{products.length} product{products.length === 1 ? '' : 's'}</p></div><div className="overflow-x-auto"><table className="min-w-full text-left text-sm"><thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500"><tr><th className="px-5 py-3">Product</th><th className="px-5 py-3">Category</th><th className="px-5 py-3">Price</th><th className="px-5 py-3">Quantity</th><th className="px-5 py-3"><span className="sr-only">Actions</span></th></tr></thead><tbody className="divide-y divide-slate-100">{isLoading && <TableSkeleton columns={5} />}{error && <tr><td colSpan={5} className="px-5 py-12 text-center text-red-600">{error}</td></tr>}{!isLoading && !error && products.length === 0 && <tr><td colSpan={5} className="px-5 py-12 text-center"><p className="font-medium text-slate-700">No products found</p><p className="mt-1 text-sm text-slate-500">Add your first product or try a different search.</p></td></tr>}{!isLoading && !error && products.map((product) => <tr key={product.id} className="hover:bg-slate-50"><td className="px-5 py-4"><p className="font-medium text-slate-900">{product.name}</p><p className="mt-0.5 font-mono text-xs text-slate-500">{product.sku}</p></td><td className="px-5 py-4 text-slate-600">{product.category.name}</td><td className="px-5 py-4 text-slate-700">{money(product.price)}</td><td className="px-5 py-4 text-slate-700">{product.quantity}</td><td className="whitespace-nowrap px-5 py-4 text-right"><button onClick={() => setModalProduct(product)} className="mr-3 text-sm font-semibold text-brand-700 hover:underline">Edit</button><button onClick={() => setProductToDelete(product)} className="text-sm font-semibold text-red-600 hover:underline">Delete</button></td></tr>)}</tbody></table></div></section>{modalProduct !== undefined && <Modal title={modalProduct ? 'Edit product' : 'Add product'} onClose={() => !isSaving && setModalProduct(undefined)}><ProductForm product={modalProduct} categories={categories} isSaving={isSaving} onSubmit={saveProduct} onCancel={() => setModalProduct(undefined)} /></Modal>}{productToDelete && <ConfirmDialog title="Delete product?" message={`This will permanently delete “${productToDelete.name}” and its stock history. This action cannot be undone.`} isBusy={isDeleting} onConfirm={confirmDelete} onClose={() => setProductToDelete(null)} />}</section>
}
